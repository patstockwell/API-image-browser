...
module.exports.models = (parsedData, directory) => {
    ...
    models.forEach((data) => {
        ...
        // if a directory was passed in, write the template to file
        if(directory) {
            writeToFile(directory, `${data.heading}.html`, htmlString)
        }
    })
    if(!directory) {
        return htmlStrings
...
module.exports.css = (directory) => {
    // First read in the file
    fs.readFile(`${__dirname}/main-style.css`, 'utf8', (err, data) => {
        if (err) {
            throw err;
        }
        // write it to disk where the html files are output
        writeToFile(directory, 'rb-style.css', data)
    })
}
...
function writeToFile(directory, fileName, content) {
    ...
    fs.writeFile(`${__dirname}/../${directory}/${fileName}`, content, function(err) {
        if(err) {
            ...
            if (parsedData.hasOwnProperty(make)) {
                ...
                for (let model in parsedData[make]) {
                    if (parsedData[make].hasOwnProperty(model)) {
                        ...
                        for(let x = 0; x < parsedData[make][model].length; x++) {
